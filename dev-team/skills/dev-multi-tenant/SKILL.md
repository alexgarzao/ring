...
> TASK: Implement TenantMiddleware using lib-commons/v3 dispatch layer package.
...
| "lib-commons v3 is too risky" | dispatch layer package only exists in v3. | **Gate 1 is MANDATORY** |
| "Core one uses organization_id" | WRONG. Core one uses tenantId → GetModulePostgresForTenant(ctx). | **Follow multi-tenant.md** |
