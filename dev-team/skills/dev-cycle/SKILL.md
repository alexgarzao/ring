...
```
❌ FORBIDDEN: Read/Write/Edit/Bash on source code directly
✅ REQUIRED:  Task(subagent_type="ring-dev-team:{agent}", ...)
```

| If You're Tempted To... | STOP. Instead... |
|------------------------|------------------|
| Read the source file | Dispatch agent - agent reads it |
| Write test file | Dispatch agent - agent writes it |
| Run `go test` | Dispatch agent - agent runs it |
| Edit implementation | Dispatch agent - agent edits it |

**You are the ORCHESTRATOR. Agents are the EXECUTORS. This is NON-NEGOTIABLE.**
