...
| 48 | Multi-Tenant Patterns | multi-tenant.md | `#multi-tenant-patterns-mandatory` | Tenant Manager API, TenantConfig data model, Generic TenantMiddleware, context functions (generic vs module-specific), Redis Lua script tenant-awareness, metrics, responsibility split, ConsumerTrigger, JWT tenantId extraction, database-per-tenant routing, testing, **MANDATORY: Both single-tenant and multi-tenant modes with backward compatibility**, Tenant Isolation Verification, M2M Credentials via Secret Manager (plugin-only) |
