...
| 48 | Multi-Tenant Patterns | multi-tenant.md | `#multi-tenant-patterns-conditional` | Tenant Manager, JWT tenantId extraction, context injection, database-per-tenant routing, testing, **⚠️ CONDITIONAL: Tenant Isolation Verification**: context-based connection checks |
...
| 50 | Multi-Tenant Patterns | multi-tenant.md | `#multi-tenant-patterns-conditional` | Tenant Manager API, TenantConfig data model, Generic TenantMiddleware, context functions (generic vs module-specific), Redis Lua script tenant-awareness, metrics, responsibility split, ConsumerTrigger, JWT tenantId extraction, database-per-tenant routing, testing, **⚠️ CONDITIONAL: Tenant Isolation Verification**: context-based connection checks |
| 51 | CORS Configuration | security.md | `#cors-configuration-mandatory` | **Configuration-driven (MANDATORY)**, production validation (no wildcard, no empty), middleware ordering (before Helmet), Helmet integration, detection commands |
