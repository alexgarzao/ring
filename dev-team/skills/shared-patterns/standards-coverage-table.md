...
| 48 | Multi-Tenant Patterns | multi-tenant.md | `#multi-tenant-patterns-conditional` | Tenant Manager API, TenantConfig data model, Generic TenantMiddleware, DualPoolMiddleware (multi-module), context functions (generic vs module-specific), Redis Lua script tenant-awareness, metrics, responsibility split, ConsumerTrigger, JWT tenant extraction, testing, **⚠️ CONDITIONAL: Tenant Isolation Verification**: IDOR prevention, detection commands |
