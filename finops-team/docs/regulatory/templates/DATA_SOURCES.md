...
|-------------|---------------|----------|
| Dados pessoais / bancários do titular | **CRM** (`crm.holder.*`, `crm.alias.*`) | Core one metadata |
...
### `midaz_onboarding`
Dados cadastrais do Core one (organizations e accounts).
...
### `midaz_transaction`
Dados transacionais do Core one (saldos, operações, rotas).
...
Campos de metadata customizados. Usados para informações regulatórias específicas não presentes no schema padrão do Core one. Definidos por cada cliente/instituição.
