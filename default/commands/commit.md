...
```bash
# ❌ WRONG - "Generated-by" text is INSIDE the -m quotes
...
git commit -m "feat: add feature" --trailer "Generated-by: Claude" --trailer "AI-Model: claude-opus-4-5-20251101"
```
...
```
git commit -m "feat: add feature" --trailer "Generated-by: Claude" --trailer "AI-Model: ..."
           │                    │ │                               │
           └── MESSAGE ─────────┘ └── SEPARATE FLAGS (NOT inside -m) ──┘
...
3. **Commit message body must be clean and professional:**
   ...
   - If you see emoji or "Generated" in your `-m` text → YOU ARE DOING IT WRONG
...
```bash
# ❌ WRONG - emoji signature in message body
git commit -m "feat: add feature
...
# ❌ WRONG - HEREDOC with signature in message body
...
# ❌ WRONG - ANY attempt to include trailers in message body
...
```bash
# ✅ CORRECT - clean message with trailers via --trailer parameter
git commit \
  -m "feat: add feature" \
  --trailer "Generated-by: Claude" \
  --trailer "AI-Model: claude-opus-4-5-20251101"
...
|-----------------|----------------|-----------------|
| "I'll put the trailer text in the message body" | `--trailer` is a GIT FLAG, not text. Text in `-m` is NOT a trailer. | **Use `--trailer "key: value"` as separate argument** |
| "The trailers need to be in the commit message" | NO. Trailers go via `--trailer` flag OUTSIDE the `-m` quotes. | **Structure: `git commit -m "msg" --trailer "..."` ** |
| "I'll format it nicely in the message body" | That's NOT a trailer - that's polluting the message body. | **NEVER put "Generated-by" text inside `-m` quotes** |
| "HEREDOC will format the trailers correctly" | HEREDOC puts everything in the message body. That's WRONG. | **Use multiple `--trailer` flags, NOT HEREDOC** |
| "The example shows trailer text in the message" | Look again. `--trailer` is OUTSIDE the `-m "..."` quotes. | **Copy the structure exactly: `-m "msg" --trailer "..."` ** |
