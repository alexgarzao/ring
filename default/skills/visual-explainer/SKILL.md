...
- You MUST generate an HTML table for any data >3 columns or 4 rows.
- You MUST use the specified Lerian color palette.
- You MUST always provide a browser-openable HTML file, never fallback to ASCII art if the threshold is met.
...
| MEDIUM | Using default Mermaid colors instead of the Lerian palette | Update CSS overrides |
| LOW | Sub-optimal font choice | Fix in next iteration |
...
| "Mermaid's default theme is fine" | Default themes look unpolished and don't match our Lerian standards. | **MUST use the custom CSS palette.** |
