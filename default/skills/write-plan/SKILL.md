...
**Announce at start:** "I'm using the ring:write-plan skill to create the implementation plan."

**Context:** This should be run in a dedicated worktree (created by ring:brainstorm skill).
...
| **Parallel session** | User opens new session, batch execution with human review → `ring:execute-plan` |
...
|---|---|---|
| Design Validation | Design phase not complete (brainstorm/PRD/TRD not validated) | STOP and report |
